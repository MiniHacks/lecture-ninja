...
export default function CustomCard({title, number, description, date, isLoading, image, id}) {
    const router = useRouter();
    return (
        <Card sx={{borderRadius: 2, my: "1rem", position: "relative"}} onClick={() =>
            !isLoading && router.push("/lecture/" + id)}>
                    ...
                    <Box sx={{
                        ...
                        <CardMedia sx={{
                            ...
                            transform: isLoading ? "scaleX(-1)" : ""
                                   ...
                                   component="video"
                                   image={isLoading ? "/img/geese_moving.gif" : process.env.NEXT_PUBLIC_BACKEND + "/file/" + id}
